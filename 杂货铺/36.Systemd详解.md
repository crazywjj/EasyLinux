...
在RHEL7之前，服务管理是分布式的被SysV init或UpStart通过/etc/rc.d/init.d下的脚本管理。这些脚本是经典的Bash脚本，允许管理员控制服务的状态。RHEL7中，这些脚本被服务单元文件替换。

CentOS 5: SysV init,串行，生成和管理用户空间中的进程以及完成系统的初始化使用的是init。

CentOS 6：Upstart,并行，借鉴ubantu，upstart,在一定程度上实现了并行启动，但是仍然存在依赖关系。

CentOS 7：Systemd,并行，借鉴MAC，真正的实现并行启动、延时按需启动。
